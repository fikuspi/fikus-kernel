 ...
 *
 *    Copyright (C) 1999 Matthew Wilcox <willy with parisc-fikus.org>
 *    Copyright (C) 2000 Philipp Rumpf
 *    Copyright (C) 2000 John Marvin <jsm with parisc-fikus.org>
 ...
 *
 *  Now, struct ustat on HP-UX is exactly the same as on Fikus, except
 *  that it contains one addition field on the end, int32_t f_blksize.
 *  So, we could have written this function to just call the Fikus
 *  sys_ustat(), (defined in fikus/fs/super.c), and then just
...
/*
 * Wrapper for hpux statfs call. At the moment, just calls the fikus native one
 ...
 *  defined in fikus/arch/i386/kernel/sys_i386.c.
 */
...
int hpux_utssys(char __user *ubuf, int n, int type)
{
	...
	case 3:
		/*  setuname():
		 *
		 *  On fikus (unlike HP-UX), utsname.nodename
		 *  is the same as the hostname.
		 *
		 *  sys_sethostname() is defined in fikus/kernel/sys.c.
		 */
		if (!capable(CAP_SYS_ADMIN))
			return -EPERM;
		/*  Unlike Fikus, HP-UX returns an error if n==0:  */
		if ( n <= 0 )
			return -EINVAL ;
		/*  Unlike Fikus, HP-UX truncates it if n is too big:  */
	...
	case 5:
		/*  gethostname():
		 *
		 *  sys_gethostname() is defined in fikus/kernel/sys.c.
		 */
		/*  Unlike Fikus, HP-UX returns an error if n==0:  */
		if ( n <= 0 )
			return -EINVAL ;
	...
	case 6:
		 ...
		 *  defined in fikus/kernel/sys.c.  If that gets updated,
	...
	case 7:
		 ...
		 *
		 *  This code should look a lot like sys_sethostname()
		 *  in fikus/kernel/sys.c.  If that gets updated, update
		 *  this code similarly.
		 */
		if (!capable(CAP_SYS_ADMIN))
			return -EPERM;
