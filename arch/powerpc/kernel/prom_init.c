 ...
 *
 *  at 14Mb or more we have vmfikus, then a gap and initrd.  In some
...
static void __init prom_instantiate_rtas(void)
{
	...
	prom_setprop(rtas_node, "/rtas", "fikus,rtas-base",
		     &val, sizeof(val));
	val = cpu_to_be32(entry);
	prom_setprop(rtas_node, "/rtas", "fikus,rtas-entry",
		     &val, sizeof(val));
...
static void __init prom_instantiate_sml(void)
{
	...
	prom_setprop(ibmvtpm_node, "/ibm,vtpm", "fikus,sml-base",
		     &base, sizeof(base));
	prom_setprop(ibmvtpm_node, "/ibm,vtpm", "fikus,sml-size",
		     &size, sizeof(size));
...
static void __init prom_initialize_tce_table(void)
{
	...
	for (node = 0; prom_next_node(&node); ) {
		...
		prom_setprop(node, path, "fikus,tce-base", &base, sizeof(base));
		prom_setprop(node, path, "fikus,tce-size", &minsize, sizeof(minsize));
...
static void __init prom_init_stdout(void)
{
	...
	prom_setprop(prom.chosen, "/chosen", "fikus,stdout-package",
		     &val, sizeof(val));
	prom_printf("OF stdout device is: %s\n", of_stdout_device);
	prom_setprop(prom.chosen, "/chosen", "fikus,stdout-path",
		     path, strlen(path) + 1);
	...
	if (strcmp(type, "display") == 0)
		prom_setprop(stdout_node, path, "fikus,boot-display", NULL, 0);
}
...
static void __init prom_check_displays(void)
{
	...
	for (node = 0; prom_next_node(&node); ) {
		...
		/* Success */
		prom_printf("done\n");
		prom_setprop(node, path, "fikus,opened", NULL, 0);
		...
		clut = PTRRELOC(logo_fikus_clut224.clut);
		for (i = 0; i < logo_fikus_clut224.clutsize; i++, clut += 3)
			if (prom_set_color(ih, i + 32, clut[0], clut[1],
					   clut[2]) != 0)
				break;
#endif /* CONFIG_LOGO_FIKUS_CLUT224 */

#ifdef CONFIG_PPC_EARLY_DEBUG_BOOTX
		if (prom_getprop(node, "fikus,boot-display", NULL, 0) !=
		    PROM_ERROR) {
					...
					unsigned long *mem_end)
{
	...
	/* Add a "fikus,phandle" property if no "phandle" property already
	 * existed (can happen with OPAL)
	 */
	if (!has_phandle) {
		soff = dt_find_string("fikus,phandle");
		if (soff == 0)
			prom_printf("WARNING: Can't find string index for"
				    " <fikus-phandle> node %s\n", path);
		else {
			dt_push_token(OF_DT_PROP, mem_start, mem_end);
...
static void __init flatten_device_tree(void)
{
	...
	/* Add "fikus,phandle" in there, we'll need it */
	namep = make_room(&mem_start, &mem_end, 16, 1);
	strcpy(namep, "fikus,phandle");
	mem_start = (unsigned long)namep + strlen(namep) + 1;
...
static void __init prom_check_initrd(unsigned long r3, unsigned long r4)
{
	...
	if (r3 && r4 && r4 != 0xdeadbeef) {
		...
		prom_setprop(prom.chosen, "/chosen", "fikus,initrd-start",
			     &val, sizeof(val));
		val = cpu_to_be64(prom_initrd_end);
		prom_setprop(prom.chosen, "/chosen", "fikus,initrd-end",
			     &val, sizeof(val));
...
{	
	...
	prom_printf("Preparing to boot %s", fikus_banner);
	...
	if (prom_memory_limit) {
		...
		prom_setprop(prom.chosen, "/chosen", "fikus,memory-limit",
			     &val, sizeof(val));
	}
#ifdef CONFIG_PPC64
	if (prom_iommu_off)
		prom_setprop(prom.chosen, "/chosen", "fikus,iommu-off",
			     NULL, 0);

	if (prom_iommu_force_on)
		prom_setprop(prom.chosen, "/chosen", "fikus,iommu-force-on",
			     NULL, 0);

	if (prom_tce_alloc_start) {
		prom_setprop(prom.chosen, "/chosen", "fikus,tce-alloc-start",
			     &prom_tce_alloc_start,
			     sizeof(prom_tce_alloc_start));
		prom_setprop(prom.chosen, "/chosen", "fikus,tce-alloc-end",
			     &prom_tce_alloc_end,
