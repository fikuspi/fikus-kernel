...
void planetcore_set_stdout_path(const char *table)
{
	...
	node = find_node_by_prop_value_str(NULL, "fikus,planetcore-label",
	                                   label);
	if (!node)
		return;
	...
	setprop_str(chosen, "fikus,stdout-path", path);
}
...
void planetcore_set_serial_speed(const char *table)
{
	...
	len = getprop(chosen, "fikus,stdout-path", prop_buf, MAX_PROP_LEN);
	if (len <= 0)
		return;
