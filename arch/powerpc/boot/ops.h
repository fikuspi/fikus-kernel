...
struct platform_ops {
	...
	void	(*exit)(void);
	void *	(*vmfikus_alloc)(unsigned long size);
};
...
static inline void *find_node_by_fikusphandle(const u32 fikusphandle)
{
	return find_node_by_prop_value(NULL, "fikus,phandle",
			(char *)&fikusphandle, sizeof(u32));
}
...
extern char _end[];
extern char _vmfikus_start[];
extern char _vmfikus_end[];
