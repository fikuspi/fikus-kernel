...
/*
 * Fikus descriptor level callbacks
 */
...

{
	 ...
	 * be translated between firmware type & fikus type.
	 */

	static unsigned char map_of_senses_to_fikus_irqtype[4] = {
	...
	*out_hwirq = intspec[0];
	if (intsize > 1) {
		...
		*out_flags = map_of_senses_to_fikus_irqtype[intspec[1] &
