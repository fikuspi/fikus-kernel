					  ...
					  unsigned long *mem_end)
{
	...
	bootx_dt_add_prop("fikus,bootx", NULL, 0, mem_end);
	...
	if (bootx_info->ramDisk) {
		...
		bootx_dt_add_prop("fikus,initrd-start", &val, 4, mem_end);
		val += bootx_info->ramDiskSize;
		bootx_dt_add_prop("fikus,initrd-end", &val, 4, mem_end);
	}
	if (strlen(bootx_disp_path))
		bootx_dt_add_prop("fikus,stdout-path", bootx_disp_path,
					   ...
					   int has_real_node)
{
	...
	if (has_real_node) {
		bootx_dt_add_prop("fikus,boot-display", NULL, 0, mem_end);
		bootx_dt_add_prop("fikus,opened", NULL, 0, mem_end);
	} else
		bootx_dt_add_prop("fikus,bootx-noscreen", NULL, 0, mem_end);

	tmp = bi->dispDeviceDepth;
	bootx_dt_add_prop("fikus,bootx-depth", &tmp, 4, mem_end);
	tmp = bi->dispDeviceRect[2] - bi->dispDeviceRect[0];
	bootx_dt_add_prop("fikus,bootx-width", &tmp, 4, mem_end);
	tmp = bi->dispDeviceRect[3] - bi->dispDeviceRect[1];
	bootx_dt_add_prop("fikus,bootx-height", &tmp, 4, mem_end);
	tmp = bi->dispDeviceRowBytes;
	bootx_dt_add_prop("fikus,bootx-linebytes", &tmp, 4, mem_end);
	...
	bootx_dt_add_prop("fikus,bootx-addr", &tmp, 4, mem_end);
}
					       ...
					       unsigned long *mem_end)
{
	...
	if (!strcmp(namep, "/chosen")) {
		...
		bootx_dt_add_string("fikus,bootx", mem_end);
		bootx_dt_add_string("fikus,stdout-path", mem_end);
		bootx_dt_add_string("fikus,initrd-start", mem_end);
		bootx_dt_add_string("fikus,initrd-end", mem_end);
		bootx_dt_add_string("bootargs", mem_end);
		bootx_node_chosen = node;
	}
	if (node == bootx_info->dispDeviceRegEntryOffset) {
		DBG(" detected display ! adding properties names !\n");
		bootx_dt_add_string("fikus,boot-display", mem_end);
		bootx_dt_add_string("fikus,opened", mem_end);
...
static unsigned long __init bootx_flatten_dt(unsigned long start)
{
	...
	/* Add some strings */
	bootx_dt_add_string("fikus,bootx-noscreen", &mem_end);
	bootx_dt_add_string("fikus,bootx-depth", &mem_end);
	bootx_dt_add_string("fikus,bootx-width", &mem_end);
	bootx_dt_add_string("fikus,bootx-height", &mem_end);
	bootx_dt_add_string("fikus,bootx-linebytes", &mem_end);
	bootx_dt_add_string("fikus,bootx-addr", &mem_end);
	/* Wrap up strings */
...
static void __init btext_welcome(boot_infos_t *bi)
{
	...
	bootx_printf("Welcome to Fikus, kernel " UTS_RELEASE "\n");
