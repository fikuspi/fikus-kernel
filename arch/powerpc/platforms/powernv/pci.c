...
static int pnv_setup_msi_irqs(struct pci_dev *pdev, int nvec, int type)
{
	...
	list_for_each_entry(entry, &pdev->msi_list, list) {
		...
		if (virq == NO_IRQ) {
			pr_warn("%s: Failed to map MSI to fikus irq\n",
				pci_name(pdev));
...
static struct iommu_table *pnv_pci_setup_bml_iommu(struct pci_controller *hose)
{
	...
	basep = of_get_property(hose->dn, "fikus,tce-base", NULL);
	sizep = of_get_property(hose->dn, "fikus,tce-size", NULL);
	...
	swinvp = of_get_property(hose->dn, "fikus,tce-sw-invalidate-info",
				 NULL);
	if (swinvp) {
