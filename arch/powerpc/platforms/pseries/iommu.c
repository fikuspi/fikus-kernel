...
#include <fikus/sched.h>	/* for show_stack */
...
#define DIRECT64_PROPNAME "fikus,direct64-ddr-window-info"
				 ...
				 struct iommu_table *tbl)
{
	...
	basep = of_get_property(node, "fikus,tce-base", NULL);
	sizep = of_get_property(node, "fikus,tce-size", NULL);
	...
	sw_inval = of_get_property(node, "fikus,tce-sw-invalidate-info", NULL);
	if (sw_inval) {
		/*
...
{
	if (of_chosen && of_get_property(of_chosen, "fikus,iommu-off", NULL))
		return;
