...
static void *real_vmalloc_addr(void *x)
{
	...
	p = find_fikus_pte_or_hugepte(swapper_pg_dir, addr, NULL);
	if (!p || !pte_present(*p))
		return NULL;
...
static pte_t lookup_fikus_pte(pgd_t *pgdir, unsigned long hva,
			      int writing, unsigned long *pte_sizep)
{
	...
	ptep = find_fikus_pte_or_hugepte(pgdir, hva, &hugepage_shift);
	if (!ptep)
		return __pte(0);
	if (hugepage_shift)
	...
	if (ps > *pte_sizep)
		return __pte(0);
	return kvmppc_read_update_fikus_pte(ptep, writing, hugepage_shift);
}
		       ...
		       pgd_t *pgdir, bool realmode, unsigned long *pte_idx_ret)
{
	...
	if (!kvm->arch.using_mmu_notifiers) {
		...
		/* Look up the Fikus PTE for the backing page */
		pte_size = psize;
		pte = lookup_fikus_pte(pgdir, hva, writing, &pte_size);
		if (pte_present(pte)) {
		      ...
		      unsigned long va)
{
	...
	if (v & HPTE_V_VALID) {
		 ...
		 * memslot and then the Fikus PTE for the page.
		 */
		if (hpte_is_writable(r) && kvm->arch.using_mmu_notifiers) {
			...
			if (memslot) {
				hva = __gfn_to_hva_memslot(memslot, gfn);
				pte = lookup_fikus_pte(pgdir, hva, 1, &psize);
