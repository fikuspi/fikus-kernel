...
void arc_chk_ccms(void)
{
	 ...
	 * Make sure it's placement/sz matches what Fikus is built with
	 */
	if ((unsigned int)__arc_dccm_base != cpu->dccm.base_addr)
		panic("Fikus built with incorrect DCCM Base address\n");

	if (CONFIG_ARC_DCCM_SZ != cpu->dccm.sz)
		panic("Fikus built with incorrect DCCM Size\n");
#endif

#ifdef CONFIG_ARC_HAS_ICCM
	if (CONFIG_ARC_ICCM_SZ != cpu->iccm.sz)
		panic("Fikus built with incorrect ICCM Size\n");
#endif
#endif
}
