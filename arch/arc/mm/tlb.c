...
void create_tlb(struct vm_area_struct *vma, unsigned long address, pte_t *ptep)
{
	 ...
	 * however Fikus only saves 1 set to save PTE real-estate
...
void arc_mmu_init(void)
{
	 ...
	 * Fikus built for MMU V2, if run on MMU V1 will break down because V1
	 *  hardware doesn't understand cmds such as WriteNI, or IVUTLB
	 * On the other hand, Fikus built for V1 if run on MMU V2 will do
