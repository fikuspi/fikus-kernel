...
enum fikus_identifier {
	bogus, ronly, removable, consin, consout, input, output
};

/* A prom device tree component. */
struct fikus_component {
	enum fikus_devclass	class;	/* node class */
	enum fikus_devtypes	type;	/* node type */
	enum fikus_identifier	iflags; /* node flags */
...
typedef struct fikus_component pcomponent;
...
union fikus_memtypes {
    enum arcs_memtypes arcs;
    enum arc_memtypes arc;
};

struct fikus_mdesc {
	union fikus_memtypes type;
	ULONG base;
	ULONG pages;
};
...
struct fikus_finfo {
	struct fikus_bigint   begin;
	struct fikus_bigint   end;
	struct fikus_bigint   cur;
	enum fikus_devtypes   dtype;
...
#define ROMVECTOR ((struct fikus_romvec *) (long)(PROMBLOCK)->romvec)

/* Cache layout parameter block. */
union fikus_cache_key {
	struct param {
#ifdef __MIPSEL__
		unsigned short size;
		unsigned char lsize;
