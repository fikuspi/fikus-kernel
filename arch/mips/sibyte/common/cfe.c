...
static void __noreturn cfe_fikus_exit(void *arg)
{
	int warm = *(int *)arg;
	...
	if (smp_processor_id()) {
		...
		if (!reboot_smp) {
			...
			reboot_smp = 1;
			smp_call_function(cfe_fikus_exit, arg, 0);
		}
	} else {
...
static void __noreturn cfe_fikus_restart(char *command)
{
	static const int zero;

	cfe_fikus_exit((void *)&zero);
}

static void __noreturn cfe_fikus_halt(void)
{
	static const int one = 1;

	cfe_fikus_exit((void *)&one);
}
...
void __init prom_init(void)
{
	...
	_machine_restart   = cfe_fikus_restart;
	_machine_halt	   = cfe_fikus_halt;
	pm_power_off = cfe_fikus_halt;
	...
	if (cfe_getenv("FIKUS_CMDLINE", arcs_cmdline, COMMAND_LINE_SIZE) < 0) {
		if (argc >= 0) {
			/* The loader should have set the command line */
			/* too early for panic to do any good */
			printk("FIKUS_CMDLINE not defined in cfe.");
			while (1) ;
		}
	}
