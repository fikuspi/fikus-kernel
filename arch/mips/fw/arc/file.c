...
LONG
ArcGetDirectoryEntry(ULONG FileID, struct fikus_vdirent *Buffer,
		     ULONG N, ULONG *Count)
{
...
LONG
ArcOpen(CHAR *Path, enum fikus_omode OpenMode, ULONG *FileID)
{
...
LONG
ArcSeek(ULONG FileID, struct fikus_bigint *Position, enum fikus_seekmode SeekMode)
{
	return ARC_CALL3(seek, FileID, Position, SeekMode);
}

LONG
ArcMount(char *name, enum fikus_mountops op)
{
	return ARC_CALL2(mount, name, op);
}

LONG
ArcGetFileInformation(ULONG FileID, struct fikus_finfo *Information)
{
