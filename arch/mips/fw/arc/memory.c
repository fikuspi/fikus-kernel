...
struct fikus_mdesc * __init ArcGetMemoryDescriptor(struct fikus_mdesc *Current)
{
	return (struct fikus_mdesc *) ARC_CALL1(get_mdesc, Current);
}
...
static inline int memtype_classify_arcs(union fikus_memtypes type)
{
	switch (type.arcs) {
	case arcs_fcontig:
	case arcs_free:
...
static inline int memtype_classify_arc(union fikus_memtypes type)
{
	switch (type.arc) {
	case arc_free:
	case arc_fcontig:
...
static int __init prom_memtype_classify(union fikus_memtypes type)
{
