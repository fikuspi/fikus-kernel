 ...
 * the system. When Fikus is started by the boot loader, the system
 * clocks are already running. Stopping a system clock during normal
 * Fikus operation should never be attempted, as peripherals that require
 ...
 *
 * Switching SYSCLK between sources during normal Fikus operation is not
