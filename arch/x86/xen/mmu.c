...
#if 0
	/* If this is a WC pte, convert back from Xen WC to Fikus WC */
 ...
 * types.
 *
 * We expect Fikus's PAT set as follows:
 *
 * Idx  PTE flags        Fikus    Xen    Default
 * 0                     WB       WB     WB
 * 1            PWT      WC       WT     WT
 * 2        PCD          UC-      UC-    UC-
 * 3        PCD PWT      UC       UC     UC
...
{
	/* We expect Fikus to use a PAT setting of
...
#if 0
	/* If Fikus is trying to set a WC pte, then map to the Xen WC.
 ...
 * the Fikus kernel and user-space are both in ring 3 while the
