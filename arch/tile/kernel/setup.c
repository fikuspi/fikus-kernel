...
/* <fikus/smp.h> doesn't provide this definition. */
#ifndef CONFIG_SMP
...
static void __init validate_hv(void)
{
	...
	if ((smp_height * smp_width) > nr_cpu_ids)
		early_panic("Hypervisor %d x %d grid too big for Fikus"
...
static void __init validate_va(void)
{
	...
	if ((long)VMALLOC_START >= 0)
		early_panic(
			"Fikus VMALLOC region below the 2GB line (%#lx)!\n"
...
void __init print_disabled_cpus(void)
{
	if (!cpumask_empty(&disabled_map)) {
		...
		pr_info("CPUs not available for Fikus: %s\n", buf);
	}
}
