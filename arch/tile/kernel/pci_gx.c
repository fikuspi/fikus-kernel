 ...
 * to Fikus which just calls handle_level_irq() after clearing the
...
int __init pcibios_init(void)
{
	...
	for (next_busno = 0, i = 0; i < num_rc_controllers; i++) {
		...
		/*
		 * Change the device ID so that Fikus bus crawl doesn't confuse
	...
	/*
	 * This comes from the generic Fikus PCI driver.
	 *
	...
	for (i = 0; i < num_rc_controllers; i++) {
		 ...
		 * that Fikus can map all of its PA space to the PCI bus.
