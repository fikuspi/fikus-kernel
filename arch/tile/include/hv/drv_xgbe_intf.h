...
typedef enum
{
  /** Inform the Fikus driver of the address of the NetIO arena memory.
   *  This offset is actually only used to convey information from netio
   *  to the Fikus driver; it never makes it from there to the hypervisor.
   *  Write-only; takes a uint32_t specifying the VA address. */
  NETIO_FIXED_ADDR               = 0x5000000000000000ULL,

  /** Inform the Fikus driver of the size of the NetIO arena memory.
   *  This offset is actually only used to convey information from netio
   *  to the Fikus driver; it never makes it from there to the hypervisor.
  ...
  /* Offsets greater than 0x7FFFFFFF can't be used directly from Fikus
 ...
 *  fikus's "MAX_SKB_FRAGS", and presumably over-estimates by one, for
...
/** FIXME: Check this from fikus, via a new "pwrite()" call. */
