...
typedef struct
{
  ...
  */
  ...
  /** Total packet buffer size.  This determines the total size, in bytes,
      ...
      default also affects the placement of buffers on Fikus NUMA nodes.
      See @ref buffer_node_weights for an explanation of buffer placement.

      In order to successfully allocate packet buffers, Fikus must have
      available huge pages on the relevant Fikus NUMA nodes.  See the
      <em>System Programmer's Guide</em> for information on configuring
      huge page support in Fikus.
   */
  ...
  /** Buffer placement weighting factors.
      ...
      on each of the available Fikus NUMA nodes.  This array is
      ...
      then all buffer space will be allocated on Fikus NUMA node zero.
