 ...
 * driver authors to proxy system calls between user space, fikus, and
 ...
 * routines.
 *
 * The IO RPC system also includes the Fikus 'iorpc' driver, which
 * proxies calls between the userspace library and the hypervisor
 * driver.  The Fikus driver is almost entirely device agnostic; it
 ...
 * order to work properly with the generic Fikus iorpc driver.
 *
 ...
 * Fikus keeps track of all buffers that might be modified by a
 ...
 * result of the Fikus driver implementation, which needs to keep
 ...
 * resource in between.  This convention keeps the Fikus driver from
 * having to track which particular devices a page is bound to.
 *
 * - At present, a memory registration is only cleared when the
 * service domain is reset.  In this case, the Fikus driver simply
 ...
 * success or failure to Fikus, which would then only decrement the
 * page reference count if the addresses were mapped.  Another scheme
 * might allow Fikus to pass a token to the HV to be returned when the
 ...
 * datatype so that the Fikus driver can validate the CPU and PL and
 ...
 * interrupt bindings which are received by Fikus but which are made
 * visible to user processes as state transitions on a file descriptor;
 * this allows user processes to use Fikus primitives, such as poll(), to
 * await particular hardware events.  This data is passed via a special
 * datatype so that the Fikus driver may recognize the pollable file
 ...
 * the Fikus driver may recognize the pollable file descriptor and
 ...
 * MEM_BUFFER(), the buffer is not registered - Fikus does not bump
 ...
 * When the user calls up to Fikus, the rpc code fills in all the fields of
 ...
 * the buffer so that the Fikus driver and generic portions of the HV iorpc
 ...
 *
 * The Fikus iorpc driver watches for system calls that contain prefix
 ...
 * that are only called by the Fikus driver, never by userspace.
 ...
 * iorpc driver.  By providing these routines in every driver, Fikus's
...
/** A structure used to describe memory registration.  Different
    ...
    description format to the next.  In particular, the Fikus iorpc
