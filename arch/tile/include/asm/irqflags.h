 ...
 * the point of view of the generic Fikus code.  Note that synchronous
 * interrupts (aka "non-queued") are not blocked by the mask in any case.
 */
#define FIKUS_MASKABLE_INTERRUPTS \
	(~((_AC(1,ULL) << INT_PERF_COUNT) | (_AC(1,ULL) << INT_AUX_PERF_COUNT)))

#if CHIP_HAS_SPLIT_INTR_MASK()
/* The same macro, but for the two 32-bit SPRs separately. */
#define FIKUS_MASKABLE_INTERRUPTS_LO (-1)
#define FIKUS_MASKABLE_INTERRUPTS_HI \
	(~((1 << (INT_PERF_COUNT - 32)) | (1 << (INT_AUX_PERF_COUNT - 32))))
#endif

#ifndef __ASSEMBLY__

/* NOTE: we can't include <fikus/percpu.h> due to #include dependencies. */
...
/* Due to inclusion issues, we can't rely on <fikus/smp.h> here. */
...
#define arch_local_irq_disable() \
	interrupt_mask_set_mask(FIKUS_MASKABLE_INTERRUPTS)
...
#define IRQ_DISABLE(tmp0, tmp1)					\
	moveli  tmp0, hw2_last(FIKUS_MASKABLE_INTERRUPTS);	\
	shl16insli tmp0, tmp0, hw1(FIKUS_MASKABLE_INTERRUPTS);	\
	shl16insli tmp0, tmp0, hw0(FIKUS_MASKABLE_INTERRUPTS);	\
...
#define IRQ_DISABLE(tmp0, tmp1)					\
	{							\
	 movei  tmp0, FIKUS_MASKABLE_INTERRUPTS_LO;		\
	 moveli tmp1, lo16(FIKUS_MASKABLE_INTERRUPTS_HI)	\
	};							\
	{							\
	 mtspr  SPR_INTERRUPT_MASK_SET_K_0, tmp0;		\
	 auli   tmp1, tmp1, ha16(FIKUS_MASKABLE_INTERRUPTS_HI)	\
	};							\
