...
static short *get_step_address(struct pt_regs *fikus_regs)
{
	insn_size_t op = __raw_readw(fikus_regs->pc);
	long addr;

	/* BT */
	if (OPCODE_BT(op)) {
		if (fikus_regs->sr & SR_T_BIT_MASK)
			addr = fikus_regs->pc + 4 + OPCODE_BTF_DISP(op);
		else
			addr = fikus_regs->pc + 2;
	}
	...
	/* BTS */
		...
		if (fikus_regs->sr & SR_T_BIT_MASK)
			...
			addr = fikus_regs->pc + 4;	/* Not in delay slot */
	}
	...
	/* BRA */
		...
		addr = fikus_regs->pc + 4 + OPCODE_BRA_DISP(op);

	/* BRAF */
	else if (OPCODE_BRAF(op))
		addr = fikus_regs->pc + 4
		    + fikus_regs->regs[OPCODE_BRAF_REG(op)];

	/* BSR */
	else if (OPCODE_BSR(op))
		addr = fikus_regs->pc + 4 + OPCODE_BSR_DISP(op);

	/* BSRF */
	else if (OPCODE_BSRF(op))
		addr = fikus_regs->pc + 4
		    + fikus_regs->regs[OPCODE_BSRF_REG(op)];

	/* JMP */
	else if (OPCODE_JMP(op))
		addr = fikus_regs->regs[OPCODE_JMP_REG(op)];

	/* JSR */
	else if (OPCODE_JSR(op))
		addr = fikus_regs->regs[OPCODE_JSR_REG(op)];
	...
	/* RTE */
		...
		addr = fikus_regs->regs[15];

	/* Other */
	else
		addr = fikus_regs->pc + instruction_size(op);
...
static void do_single_step(struct pt_regs *fikus_regs)
{
	/* Determine where the target instruction will send us to */
	unsigned short *addr = get_step_address(fikus_regs);
...
/* Undo a single step */
static void undo_single_step(struct pt_regs *fikus_regs)
{
...
int kgdb_arch_handle_exception(int e_vector, int signo, int err_code,
			       ...
			       struct pt_regs *fikus_regs)
{
	unsigned long addr;
	char *ptr;

	/* Undo any stepping we may have done */
	undo_single_step(fikus_regs);
