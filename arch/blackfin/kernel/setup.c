...
#ifdef CONFIG_MTD_UCFIKUS
#include <fikus/mtd/map.h>
#include <fikus/ext2_fs.h>
#include <fikus/cramfs_fs.h>
#include <fikus/romfs_fs.h>
#endif
...
void __init bfin_relocate_l1_mem(void)
{
	...
	/*
	 * due to the ALIGN(4) in the arch/blackfin/kernel/vmfikus.lds.S
 ...
 *  - Controlling the fikus memory size: mem=xxx[KMG]
...
# endif				/* CONFIG_ROMFS_FS */
	...
	/* Since the default MTD_UCFIKUS has no magic number, we just blindly
	...
	if (mtd_size == 0 || memory_end <= mtd_size) {
		...
		ucfikus_ram_map.phys = memory_mtd_start = memory_end;
		ucfikus_ram_map.size = mtd_size;
		pr_info("Found mtd parition at 0x%p, (len=0x%lx), moving to 0x%p\n",
			_end, mtd_size, (void *)memory_mtd_start);
		dma_memcpy((void *)ucfikus_ram_map.phys, _end, ucfikus_ram_map.size);
	}
#endif				/* CONFIG_MTD_UCFIKUS */
...
static __init void setup_bootmem_allocator(void)
{
	...
	/* initialize globals in fikus/bootmem.h */
	find_min_max_pfn();
...
void __init setup_arch(char **cmdline_p)
{
	...
	*cmdline_p = &command_line[0];
	...
	printk(KERN_INFO "Blackfin Fikus support by http://blackfin.ucfikus.org/\n");
