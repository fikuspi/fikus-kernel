/*
 *  fikus/arch/sparc/kernel/setup.c
 *
 ...
 */
void __init sparc32_start_kernel(struct fikus_romvec *rp)
{
	prom_init(rp);
...
void __init setup_arch(char **cmdline_p)
{
	...
	*cmdline_p = prom_getbootargs();
	...
	if((boot_flags&BOOTME_DEBUG) && (fikus_dbvec!=0) && 
	   ((*(short *)fikus_dbvec) != -1)) {
		printk("Booted under KADB. Syncing trap table.\n");
		(*(fikus_dbvec->teach_debugger))();
	}
