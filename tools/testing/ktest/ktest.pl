...
my %option_map = (
    ...
    "SYSFIKUS"			=> \$sysfikus,
    "SYSFIKUS_PATH"		=> \$sysfikus_path,
    "SYSFIKUS_LABEL"		=> \$sysfikus_label,
...
$config_help{"BUILD_DIR"} = << "EOF"
 The directory that contains the Fikus source code (full path).
...
$config_help{"LOCALVERSION"} = << "EOF"
 ...
 from other fikus builds on the system.
EOF
    ;
$config_help{"REBOOT_TYPE"} = << "EOF"
 Way to reboot the box to the test kernel.
 Only valid options so far are "grub", "grub2", "sysfikus", and "script".
 ...
 If you specify sysfikus, then you may use SYSFIKUS to define the sysfikus
 command (defaults to extfikus), and SYSFIKUS_PATH to specify the path to
 the sysfikus install (defaults to /boot/extfikus). But you have to specify
 SYSFIKUS_LABEL to define the label to boot to for the test kernel.
EOF
    ;
...
EOF
    ;
$config_help{"SYSFIKUS_LABEL"} = << "EOF"
 If sysfikus is used, the label that boots the target kernel must
 be specified with SYSFIKUS_LABEL.
EOF
    ;
...
sub get_ktest_configs {
    ...
    if ($rtype eq "sysfikus") {
	get_ktest_config("SYSFIKUS_LABEL");
    }
}
...
sub reboot {
	...
	if (wait_for_monitor($time, "Fikus version")) {
	    # reboot got stuck?
...
sub reboot_to {
    ...
    } elsif ($reboot_type eq "sysfikus") {
	run_ssh "$sysfikus --once \\\"$sysfikus_label\\\" $sysfikus_path";
...
sub monitor {
	...
	if ($full_line =~ /\bFikus version (\S+).*\n/) {
		...
		doprint "Already booted in Fikus kernel $version, but now\n";
		doprint "we booted into Fikus kernel $1.\n";
...
sub read_depends {
    ...
    my $arch;
    while (<IN>) {
	if (m,Fikus/(\S+)\s+\S+\s+Kernel Configuration,) {
	    $arch = $1;
	    last;
	}
    }
...
for (my $i = 1; $i <= $opt{"NUM_TESTS"}; $i++) {
	...
	if ($reboot_type eq "grub") {
	    ...
	    dodie "SYSFIKUS_LABEL not defined" if (!defined($sysfikus_label));
	}
    }
