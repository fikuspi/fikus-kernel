...
static int do_test_code_reading(bool try_kcore)
{
	...
	pid_t pid;
	struct map *map;
	bool have_vmfikus, have_kcore, excl_kernel = false;
	...
	/* Force the use of kallsyms instead of vmfikus to try kcore */
	if (try_kcore)
	...
	have_vmfikus = dso__is_vmfikus(map->dso);
	...
	if (!have_vmfikus && !have_kcore && !try_kcore)
		err = TEST_CODE_READING_NO_KERNEL_OBJ;
	else if (!have_vmfikus && !try_kcore)
		err = TEST_CODE_READING_NO_VMFIKUS;
	else if (excl_kernel)
...
int test__code_reading(void)
{
	...
	case TEST_CODE_READING_OK:
		return 0;
	case TEST_CODE_READING_NO_VMFIKUS:
		fprintf(stderr, " (no vmfikus)");
		return 0;
