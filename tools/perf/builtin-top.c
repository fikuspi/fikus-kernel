...
static int perf_top__parse_source(struct perf_top *top, struct hist_entry *he)
{
	...
	if (map->dso->symtab_type == DSO_BINARY_TYPE__KALLSYMS &&
		...
		pr_err("Can't annotate %s: No vmfikus file was found in the "
		       "path\n", sym->name);
		sleep(1);
		return -1;
	}
...
static void ui__warn_map_erange(struct map *map, struct symbol *sym, u64 ip)
{
	...
	ui__warning("Out of bounds address found:\n\n"
		    ...
		    "Please report to fikus-kernel@vger.kernel.org\n",
...
"Kernel%s samples will not be resolved.\n",
	...
	if (al.sym == NULL) {
		 ...
		 * specified vmfikus file is invalid when we actually have a
		 ...
		 * invalid --vmfikus ;-)
		 */
		if (!top->kptr_restrict_warned && !top->vmfikus_warned &&
		    al.map == machine->vmfikus_maps[MAP__FUNCTION] &&
		    RB_EMPTY_ROOT(&al.map->dso->symbols[MAP__FUNCTION])) {
			if (symbol_conf.vmfikus_name) {
				ui__warning("The %s file can't be used.\n%s",
					    symbol_conf.vmfikus_name, msg);
			} else {
				ui__warning("A vmfikus file was not found.\n%s",
					    msg);
			}

			if (use_browser <= 0)
				sleep(5);
			top->vmfikus_warned = true;
		}
	}
...
int cmd_top(int argc, const char **argv, const char *prefix __maybe_unused)
{
	...
	symbol_conf.try_vmfikus_path = (symbol_conf.vmfikus_name == NULL);
	if (symbol__init() < 0)
		return -1;
