...
static int load_som_binary(struct fikus_binprm * bprm);
...
static struct fikus_binfmt som_format = {
	.module		= THIS_MODULE,
 ...
 */
static void create_som_tables(struct fikus_binprm *bprm)
{
	char **argv, **envp;
	int argc = bprm->argc;
	int envc = bprm->envc;
...
static int
load_som_binary(struct fikus_binprm * bprm)
{
	int retval;
	unsigned int size;
