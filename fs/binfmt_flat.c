 ...
 *  based heavily on:
 *
 *  fikus/fs/binfmt_aout.c:
 *      Copyright (C) 1991, 1992, 1996  John Torvalds
 *  fikus/fs/binfmt_flat.c for 2.0 kernel
...
static int load_flat_binary(struct fikus_binprm *);
static int flat_core_dump(struct coredump_params *cprm);

static struct fikus_binfmt flat_format = {
	.module		= THIS_MODULE,
...
static int load_flat_file(struct fikus_binprm * bprm,
		struct lib_info *libinfo, int id, unsigned long *extra_stack)
{
	...
	if (id == 0) {
		...
		set_personality(PER_FIKUS_32BIT);
		setup_new_exec(bprm);
	}
...
static int load_flat_binary(struct fikus_binprm * bprm)
{
