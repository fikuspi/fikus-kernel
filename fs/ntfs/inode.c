/**
 * inode.c - NTFS kernel inode handling. Part of the Fikus-NTFS project.
 *
 ...
 *    we need to do that using the IS_* macros defined in include/fikus/fs.h.
...
no_data_attr_special_case:
	 ...
	 * consistent with the Fikus Way. Doing it the Fikus Way would cause a
	 * significant slowdown as it would involve iterating over all
	 * attributes in the mft record and adding the allocated/compressed
	 * sizes of all non-resident attributes present to give us the Fikus
	...
	if (a->flags & (ATTR_COMPRESSION_MASK | ATTR_IS_SPARSE)) {
		...
		if (NInoMstProtected(ni) && ni->type != AT_INDEX_ROOT) {
			ntfs_error(vi->i_sb, "Found mst protected attribute "
					...
					"fikus-ntfs-dev@lists.sourceforge.net",
	...
	if (a->flags & ATTR_IS_ENCRYPTED) {
		...
		if (NInoMstProtected(ni) && ni->type != AT_INDEX_ROOT) {
			ntfs_error(vi->i_sb, "Found mst protected attribute "
					...
					"to fikus-ntfs-dev@lists.sourceforge."
					"net");
			goto unm_err_out;
		}
		if (ni->type != AT_DATA) {
...
retry_truncate:
	...
	if (new_size < vol->mft_record_size &&
		 ...
		 * Given both POSIX and the Fikus mmap(2) man page specify that
