...
static int dst_get_device_id(struct dst_state *state)
{
	...
	if (i >= ARRAY_SIZE(dst_tlist)) {
		...
		dprintk(verbose, DST_ERROR, 1, "please email fikus-dvb@fikustv.org with this type in");
...
struct dst_state *dst_attach(struct dst_state *state, struct dvb_adapter *dvb_adapter)
{
	...
	case DST_TYPE_IS_ATSC:
		...
		break;
	default:
		dprintk(verbose, DST_ERROR, 1, "unknown DST type. please report to the FikusTV.org DVB mailinglist.");
		kfree(state);
		return NULL;
	}
