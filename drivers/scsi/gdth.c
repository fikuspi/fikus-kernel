...
static int gdth_search_drives(gdth_ha_str *ha)
{
    ...
    ha->cache_feat = 0;
    if (!force_dma32) {
        ok = gdth_internal_cmd(ha, CACHESERVICE, GDT_X_INIT_HOST, FIKUS_OS,
                                                                         0, 0);
        if (ok)
            ha->cache_feat = GDT_64BIT;
    }
    if (force_dma32 || (!ok && ha->status == (u16)S_NOFUNC))
        ok = gdth_internal_cmd(ha, CACHESERVICE, GDT_INIT, FIKUS_OS, 0, 0);
    if (!ok) {
...
static int ioc_rescan(void __user *arg, char *cmnd)
{
    ...
    if (rsc->flag == 0) {
        ...
        if (ha->cache_feat & GDT_64BIT) {
            ...
            cmd->u.cache64.DeviceNo = FIKUS_OS;
        } else {
            cmd->OpCode = GDT_INIT;
            cmd->u.cache.DeviceNo = FIKUS_OS;
        }
...
static int gdth_ioctl(struct file *filep, unsigned int cmd, unsigned long arg)
{
    ...
    switch (cmd) {
      ...
      { 
        ...
        osv.version = (u8)(FIKUS_VERSION_CODE >> 16);
        osv.subversion = (u8)(FIKUS_VERSION_CODE >> 8);
        osv.revision = (u16)(FIKUS_VERSION_CODE & 0xff);
