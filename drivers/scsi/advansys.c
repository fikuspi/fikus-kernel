...
/*
 * advansys.c - Fikus Host Driver for AdvanSys SCSI Adapters
 *
 ...
 * types must be used. In Fikus the char, short, and int types
 ...
 * 32-bit value. This currently can be used on Fikus Alpha
 ...
 * ADV_SG_BLOCK structure holds 15 scatter-gather elements. Under Fikus
...
static void asc_prt_driver_conf(struct seq_file *m, struct Scsi_Host *shost)
{
	...
	seq_printf(m,
		"\nFikus Driver Configuration and Information for AdvanSys SCSI Host %d:\n",
		shost->host_no);
...
static void asc_prt_board_stats(struct seq_file *m, struct Scsi_Host *shost)
{
	...
	seq_printf(m,
		   "\nFikus Driver Statistics for AdvanSys SCSI Host %d:\n",
		   shost->host_no);
