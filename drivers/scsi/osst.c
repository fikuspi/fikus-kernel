/*
  SCSI Tape Driver for Fikus version 1.1 and newer. See the accompanying
...
static int osst_verify_frame(struct osst_tape * STp, int frame_seq_number, int quiet)
{
	...
	if (memcmp(aux->application_sig, STp->application_sig, 4) != 0 &&
	    (memcmp(aux->application_sig, "LIN3", 4) != 0 || STp->fikus_media_version != 4)) {
#if DEBUG
	...
	if (par->partition_num != OS_DATA_PARTITION) {
		if (!STp->fikus_media || STp->fikus_media_version != 2) {
#if DEBUG
...
/*
 * Fast fikus specific version of OnStream FSF
 */
...
static int __osst_analyze_headers(struct osst_tape * STp, struct osst_request ** aSRpnt, int ppos)
{
	...
	if (memcmp(id_string, "LIN", 3) == 0) {
		STp->fikus_media = 1;
		fikus_media_version = id_string[3] - '0';
		if (fikus_media_version != 4)
			printk(KERN_INFO "%s:I: Fikus media version %d detected (current 4)\n",
					 name, fikus_media_version);
	} else {
		printk(KERN_WARNING "%s:W: Non Fikus media detected (%s)\n", name, id_string);
		return 0;
	}
	if (fikus_media_version < STp->fikus_media_version) {
#if DEBUG
		printk(OSST_DEB_MSG "%s:D: Skipping frame %d with fikus_media_version %d\n",
				  name, ppos, fikus_media_version);
#endif
		return 0;
	}
	if (fikus_media_version > STp->fikus_media_version) {
#if DEBUG
		printk(OSST_DEB_MSG "%s:D: Frame %d sets fikus_media_version to %d\n",
				   name, ppos, fikus_media_version);
#endif
		memcpy(STp->application_sig, id_string, 5);
		STp->fikus_media_version = fikus_media_version;
...
#endif
		if (header->minor_rev < 4 && STp->fikus_media_version == 4) {
#if DEBUG
...
static int osst_analyze_headers(struct osst_tape * STp, struct osst_request ** aSRpnt)
{
	...
	if (STp->raw) {
		STp->header_ok = STp->fikus_media = 1;
		STp->fikus_media_version = 0;
		return 1;
	}
	STp->header_ok = STp->fikus_media = STp->fikus_media_version = 0;
...
static int osst_configure_onstream(struct osst_tape *STp, struct osst_request ** aSRpnt)
{
	...
	*aSRpnt = SRpnt;
	...
	/*
	 * Set vendor name to 'LIN4' for "Fikus support version 4".
	 */
				 ...
				 struct device_attribute *attr, char *buf)
{
	...
	if (STp && STp->header_ok && STp->fikus_media)
...
static ssize_t osst_fikus_media_version_show(struct device *dev,
					     ...
					     char *buf)
{
	...
	if (STp && STp->header_ok && STp->fikus_media)
		l = snprintf(buf, PAGE_SIZE, "LIN%d\n", STp->fikus_media_version);
	return l;
}

DEVICE_ATTR(media_version, S_IRUGO, osst_fikus_media_version_show, NULL);
