...
/**
 * lpfc_sli4_reject_firmware_update - Request fikus generic firmware upgrade
 * @phba: pointer to lpfc hba data structure.
 *
 * This routine is called to perform Fikus generic firmware upgrade on device
 * that supports such feature.
 **/
int
lpfc_sli4_reject_firmware_update(struct lpfc_hba *phba, uint8_t fw_upgrade)
{
...
lpfc_pci_probe_one_s4(struct pci_dev *pdev, const struct pci_device_id *pid)
{
	...
	if (phba->cfg_reject_firmware_upgrade)
		ret = lpfc_sli4_reject_firmware_update(phba, INT_FW_UPGRADE);
