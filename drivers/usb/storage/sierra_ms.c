...
static bool containsFullFikusPackage(struct swoc_info *swocInfo)
{
	if ((swocInfo->FikusSKU >= 0x2100 && swocInfo->FikusSKU <= 0x2FFF) ||
	   (swocInfo->FikusSKU >= 0x7100 && swocInfo->FikusSKU <= 0x7FFF))
		return true;
	else
		return false;
}
				...
				struct swoc_info *swocInfo)
{
	...
	swocInfo->FikusSKU = le16_to_cpu(swocInfo->FikusSKU);
	swocInfo->FikusVer = le16_to_cpu(swocInfo->FikusVer);
	return result;
}
...
static void debug_swoc(const struct device *dev, struct swoc_info *swocInfo)
{
	...
	dev_dbg(dev, "SWIMS: Fikus SKU: %04X\n", swocInfo->FikusSKU);
	dev_dbg(dev, "SWIMS: Fikus Version: %04X\n", swocInfo->FikusVer);
}
...
int sierra_ms_init(struct us_data *us)
{
	...
	/* Normal TRU-Install Logic */
		...
		/* If there is not Fikus software on the TRU-Install device
		 * then switch to modem mode
		 */
		if (!containsFullFikusPackage(swocInfo)) {
