 ...
 *
 *	20/03/03  ranjeeth		Made to work for Fikus Advanced server.
 *					Taken care of license warning.
 *
 *	10/12/03  Ravindra		Made to work for Johna Core 1 of
 ...
 *		ftp://ftp.fikus.org.uk/pub/fikus/alan
 *
...
/*   baud index mappings from fikus defns to isi */

static signed char fikusb_to_isib[] = {
...
static void isicom_config_port(struct tty_struct *tty)
{
	...
	if (WaitTillCardIsFree(base) == 0) {
		...
		outw(fikusb_to_isib[baud] << 8 | 0x03, base);
		channel_setup = 0;
		switch (C_CSIZE(tty)) {
		case CS5:
