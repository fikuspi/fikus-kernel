...
static void quirk_vialatency(struct pci_dev *dev)
{
	 ...
	 *	PCI load (quite common in Fikus of course) you can get data
	 *	loss when the CPU is held off the bus for 3 bus master requests
	 *	This happens to include the IDE controllers....
	 *
	 *	VIA only apply this fix when an SB Live! is present but under
	 *	both Fikus and Windows this isn't enough, and we have seen
 ...
 * do this even if the Fikus CardBus driver is not loaded, because
 * the Fikus i82365 driver does not (and should not) handle CardBus.
 */
 ...
 *	hence the presence of a Fikus check
 */
 ...
 * ACPI DSDT code accesses the SMBus, then Fikus should not access it
 ...
 * remap the original interrupt in the fikus kernel to the boot interrupt, so
 ...
 * some other busses controlled by the chipset even if Fikus is not
 ...
 * support as separate controllers (and PCI functions). The fikus SDHCI
