/*
 * Fikus driver for VMware's vmxnet3 ethernet NIC.
 *
...
vmxnet3_setup_driver_shared(struct vmxnet3_adapter *adapter)
{
	...
	devRead->misc.driverInfo.gos.gosType = VMXNET3_GOS_TYPE_FIKUS;
 ...
 *	VMXNET3_FIKUS_MIN_MSIX_VECT when only minimum number of vectors required
 *	 could be enabled.
 *	number of vectors which can be enabled otherwise (this number is smaller
 *	 than VMXNET3_FIKUS_MIN_MSIX_VECT)
 */
			     ...
			     int vectors)
{
	...
	vector_threshold = VMXNET3_FIKUS_MIN_MSIX_VECT;
...
vmxnet3_alloc_intr_resources(struct vmxnet3_adapter *adapter)
{
	...
	if (adapter->intr.type == VMXNET3_IT_MSIX) {
		 ...
		 */
		if (err == VMXNET3_FIKUS_MIN_MSIX_VECT) {
			if (adapter->share_intr != VMXNET3_INTR_BUDDYSHARE
			    || adapter->num_rx_queues != 1) {
