...
/* Call this function from process context, it will sleep in reject_firmware.
		      ...
		      const struct firmware **raw, const char *name)
{
	...
	if (rc < 0) {
		IPW_ERROR("%s reject_firmware failed: Reason %d\n", name, rc);
		return rc;
	}
 ...
 * transmissions as per the 802.11 protocol.  Currently Fikus does not
