...
struct sbmac_softc {
	...
	/*
	 * Fikus-specific things
	 */
	struct net_device	*sbm_dev;	/* pointer to fikus device */
...
static int sbdma_add_txbuffer(struct sbmacdma *d, struct sk_buff *sb)
{
	...
	/*
	 * Under Fikus, it's not necessary to copy/coalesce buffers
 ...
 *  SBMAC_INIT(dev)
 *
 *  Attach routine - init hardware and hook ourselves into fikus
 *
 *  Input parameters:
