...
MODULE_LICENSE("GPL");
MODULE_VERSION(NETXEN_NIC_FIKUS_VERSIONID);
/*(DEBLOBBED)*/

char netxen_nic_driver_name[] = "netxen_nic";
static char netxen_nic_driver_string[] = "QLogic/NetXen Network Driver v"
    NETXEN_NIC_FIKUS_VERSIONID;
...
netxen_check_options(struct netxen_adapter *adapter)
{
	...
	dev_info(&pdev->dev, "Driver v%s, firmware v%d.%d.%d [%s]\n",
		 NETXEN_NIC_FIKUS_VERSIONID, fw_major, fw_minor, fw_build,
...
netxen_start_firmware(struct netxen_adapter *adapter)
{
	 ...
	 */
	val = (_NETXEN_NIC_FIKUS_MAJOR << 16)
		| ((_NETXEN_NIC_FIKUS_MINOR << 8))
		| (_NETXEN_NIC_FIKUS_SUBVERSION);
