 ...
 * allows an app to sit between the actual link and fikus, passing
 * (some) packets along to fikus, and forwarding (some) packets sent
 * out by fikus.
 */
...
static struct sk_buff *mpipe_buf_to_skb(void *va)
{
	...
	if (skb->data != va) {
		 ...
		 */
		panic("Corrupt fikus buffer! va=%p, skb=%p, skb->data=%p",
		      va, skb, skb->data);
	}
...
/* Provide fikus buffers to mPIPE. */
...
/* Sync mPIPE's timestamp up with Fikus system time and register PTP clock. */
...
fail:
	...
	/* Don't return raw gxio error codes to generic Fikus. */
