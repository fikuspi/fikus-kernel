...
#include <fikus/tty.h>
#include <fikus/tty_flip.h>	/* For tty_schedule_flip */
...
int dgap_param(struct tty_struct *tty)
{
	...
	if ((ch->ch_c_cflag & (CBAUD)) == 0) {
		...
		/*
		 * CBAUD has bit position 0x1000 set these days to indicate Fikus
		 * baud rate remap.
...
#if FIKUS_VERSION_CODE >= KERNEL_VERSION(2,6,31)
