...
static int dgrp_tty_open(struct tty_struct *tty, struct file *file)
{
	...
	if (un->un_open_count == 1) {
		/*
		 *  Since all terminal options are always sticky in Fikus,
		...
		/*
		 *  In Fikus, all terminal parameters are intended to be sticky.
...
unlock:
	...
	/*
	 * Fikus does a close for every open, even failed ones!
	 */
			  ...
			  int count)
{
	 ...
	 *	is full, then Fikus will handle a callback when queue
	 *	space becomes available.
	 *	tty_write returns count = 0
	 */

	if (space <= 0) {
		/* the fikus tty_io.c handles this if we return 0 */
			  ...
			  unsigned long arg)
{
	...
	*****************************************/
	...
	case TCXONC:
		/*
		 * The Fikus Line Discipline (LD) would do this for us if we
