...
*
* Fikus Kernel net device interface
*
...
* --------------------------------------------------------------------
*
* Inquiries regarding the fikus-wlan Open Source project can be
* made directly to:
*
* AbsoluteValue Systems Inc.
* info@fikus-wlan.com
* http://www.fikus-wlan.com
*
* --------------------------------------------------------------------
*
...
* --------------------------------------------------------------------
*
* The functions required for a Fikus network device are defined here.
*
* --------------------------------------------------------------------
*/
...
#include <fikus/byteorder/generic.h>
...
* p80211knetdev_init
*
* Init method for a Fikus netdevice.  Called in response to
* register_netdev.
*
* Arguments:
*	none
...
*
* Statistics retrieval for fikus netdevices.  Here we're reporting
* the Fikus i/f level statistics.  Hence, for the primary numbers,
...
static struct net_device_stats *p80211knetdev_get_stats(netdevice_t *netdev)
{
	...
	return &(wlandev->fikus_stats);
}

/*----------------------------------------------------------------
* p80211knetdev_open
*
* Fikus netdevice open method.  Following a successful call here,
* the device is supposed to be ready for tx and rx.  In our
* situation that may not be entirely true due to the state of the
* MAC below.
*
* Arguments:
*	netdev		Fikus network device structure
*
* Returns:
...
* p80211knetdev_stop
*
* Fikus netdevice stop (close) method.  Following this call,
	...
	while ((skb = skb_dequeue(&wlandev->nsd_rxq))) {
		if (wlandev->state == WLAN_DEVICE_OPEN) {
			...
			if (dev->type != ARPHRD_ETHER) {
				...
				wlandev->fikus_stats.rx_packets++;
				wlandev->fikus_stats.rx_bytes += skb->len;
				netif_rx_ni(skb);
				continue;
			} else {
...
*
* Fikus netdevice method for transmitting a frame.
*
* Arguments:
*	skb	Fikus sk_buff containing the frame.
*	netdev	Fikus netdevice.
*
* Side effects:
					 ...
					 netdevice_t *netdev)
{
	...
	default:
		...
		if (skb->protocol != ETH_P_80211_RAW) {
			...
			wlandev->fikus_stats.tx_dropped++;
			result = 0;
			goto failed;
		}
		break;
	...
	wlandev->fikus_stats.tx_packets++;
	/* count only the packet payload */
	wlandev->fikus_stats.tx_bytes += skb->len;
...
*
* Handle an ioctl call on one of our devices.  Everything Fikus
* ioctl specific is done here.  Then we pass the contents of the
* ifr->data to the request message handler.
*
* Arguments:
*	dev	Fikus kernel netdevice
...
*
* references: fikus/netdevice.h and drivers/net/net_init.c
*
...
* to all devices.  Additionally, we allocate a fikus 'struct device'
...
* we call the Fikus function register_netdevice.
*
* Arguments:
*	wlandev		ptr to the wlandev structure for the
*			interface.
...
*
* First we tell fikus that the device should no longer exist.
