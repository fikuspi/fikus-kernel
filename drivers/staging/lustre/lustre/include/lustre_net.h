...
#include <fikus/lustre_net.h>

#include <fikus/libcfs/libcfs.h>
// #include <obd.h>
#include <fikus/lnet/lnet.h>
 ...
 * kmem_cache_create() to complain (on Fikus), so the erroneous situation
...
struct nrs_orr_data {
	 ...
	 * versions of Fikus by Lustre, kmem_cache_create() just sets a pointer
	 * to the name string provided. kstrdup() is used in the version of
	 * kmeme_cache_create() in current Fikus mainline, so we may be able to
