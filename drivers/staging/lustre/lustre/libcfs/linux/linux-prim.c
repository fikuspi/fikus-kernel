...
#define FIKUS_WAITQ(w) ((wait_queue_t *) w)
#define FIKUS_WAITQ_HEAD(w) ((wait_queue_head_t *) w)

void
init_waitqueue_entry_current(wait_queue_t *link)
{
	init_waitqueue_entry(FIKUS_WAITQ(link), current);
}
EXPORT_SYMBOL(init_waitqueue_entry_current);

/**
 * wait_queue_t of Fikus (version < 2.6.34) is a FIFO list for exclusively
...
add_wait_queue_exclusive_head(wait_queue_head_t *waitq, wait_queue_t *link)
{
	...
	spin_lock_irqsave(&FIKUS_WAITQ_HEAD(waitq)->lock, flags);
	__add_wait_queue_exclusive(FIKUS_WAITQ_HEAD(waitq), FIKUS_WAITQ(link));
	spin_unlock_irqrestore(&FIKUS_WAITQ_HEAD(waitq)->lock, flags);
}
