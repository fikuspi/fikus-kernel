 ...
 *
 * libcfs/libcfs/fikus/fikus-curproc.c
 *
 * Lustre curproc API implementation for Fikus kernel
 *
...
{
#if defined (_FIKUS_CAPABILITY_VERSION) && _FIKUS_CAPABILITY_VERSION == 0x19980330
	*cap = cfs_cap_pack(kcap);
#elif defined (_FIKUS_CAPABILITY_VERSION) && _FIKUS_CAPABILITY_VERSION == 0x20071026
 ...
 * userspace binary interfaces.  On 32-bit Fikus this is (unfortunately)
 * always true, even if the application is using LARGEFILE64 and 64-bit
 * APIs, because Fikus provides no way for the filesystem to know if it
