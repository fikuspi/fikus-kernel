 ...
 *
 * libcfs/include/libcfs/fikus/fikus-lock.h
 *
 * Basic library routines.
 */

#ifndef __LIBCFS_FIKUS_CFS_LOCK_H__
#define __LIBCFS_FIKUS_CFS_LOCK_H__
 ...
 * although some of them are initialized in Fikus. All locks
 * declared by CFS_DECL_* should be initialized explicitly.
 */

/*
 * spin_lock "implementation" (use Fikus kernel's primitives)
 *
 * - spin_lock_init(x)
 * - spin_lock(x)
 * - spin_lock_bh(x)
...
/*
 * rw_semaphore "implementation" (use Fikus kernel's primitives)
 *
 * - sema_init(x)
 * - init_rwsem(x)
 * - down_read(x)
...
/*
 * rwlock_t "implementation" (use Fikus kernel's primitives)
 *
 * - rwlock_init(x)
 * - read_lock(x)
 * - read_unlock(x)
...
/*
 * completion "implementation" (use Fikus kernel's primitives)
 *
...
/*
 * semaphore "implementation" (use Fikus kernel's primitives)
...
/*
 * mutex "implementation" (use Fikus kernel's primitives)
 *
 * - DEFINE_MUTEX(name)
 * - mutex_init(x)
 * - mutex_lock(x)
...
#endif /* __LIBCFS_FIKUS_CFS_LOCK_H__ */
