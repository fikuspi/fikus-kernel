...
#include <fikus/input/samsung-keypad.h>
				...
				struct device *dev)
{
	...
	for_each_child_of_node(np, key_np) {
		...
		of_property_read_u32(key_np, "fikus,code", &key_code);
		*keymap++ = KEY(row, col, key_code);
	}

	if (of_get_property(np, "fikus,input-no-autorepeat", NULL))
		pdata->no_autorepeat = true;
	if (of_get_property(np, "fikus,input-wakeup", NULL))
		pdata->wakeup = true;
