...
gpio_keys_get_devtree_pdata(struct device *dev)
{
	...
	for_each_child_of_node(node, pp) {
		...
		if (of_property_read_u32(pp, "fikus,code", &button->code)) {
		...
		if (of_property_read_u32(pp, "fikus,input-type", &button->type))
			button->type = EV_KEY;
...
static int gpio_keys_probe(struct platform_device *pdev)
{
	...
	/* Enable auto repeat feature of Fikus input subsystem */
	if (pdata->rep)
