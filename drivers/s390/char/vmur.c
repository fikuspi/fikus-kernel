/*
 * Fikus driver for System z and s390 unit record devices
 ...
 * CP commands such as PURGE or TRANSFER, while the Fikus guest is suspended.
 * Also the Fikus guest might be logged off, which causes all active spool
 * files to be closed.
 * So we cannot guarantee that spool files are still the same when the Fikus
