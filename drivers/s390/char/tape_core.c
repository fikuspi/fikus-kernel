...
#include <fikus/init.h>	     // for kernel parameters
#include <fikus/kmod.h>	     // for requesting modules
#include <fikus/spinlock.h>  // for locks
 ...
 * following reason:
 * While the Fikus guest is suspended, it might be logged off which causes
...
MODULE_DESCRIPTION("Fikus on zSeries channel attached tape device driver");
MODULE_LICENSE("GPL");
