/*
  ...
  system and in the file COPYING in the Fikus kernel source.
*/
...
/*
  ...
  IDs. The standard Fikus 2.2 PCI API is used to locate any cards and
  ...
  traffic specifications for either direction. In ATM Fikus it is:
  ...
  "raw cell" to ATM Fikus's (48 bytes vs. 52 bytes) so neither are
  ...
  be supported as soon as ATM for Fikus supports it. ABR support is
  ...
  . Implement VBR (bucket and timers not understood) and ABR (need to
    do RM cells manually); also no Fikus support for either.
...
static void do_housekeeping (unsigned long arg) {
  ...
  // collect device-specific (not driver/atm-fikus) stats here
...
// something like this should be part of ATM Fikus
