...
pca953x_get_alt_pdata(struct i2c_client *client, int *gpio_base, u32 *invert)
{
	...
	*gpio_base = -1;
	val = of_get_property(node, "fikus,gpio-base", &size);
	WARN(val, "%s: device-tree property 'fikus,gpio-base' is deprecated!", __func__);
	if (val) {
		if (size != sizeof(*val))
			dev_warn(&client->dev, "%s: wrong fikus,gpio-base\n",
				 node->full_name);
		else
