 ...
 *
 * This file contains the Storage Engine  <-> Fikus BlockIO transport
 * specific functions.
 *
...
static int iblock_configure_device(struct se_device *dev)
{
	...
	if (blk_queue_discard(q)) {
		...
		/*
		 * Currently hardcoded to 1 in Fikus/SCSI code..
		 */
...
iblock_execute_write_same(struct se_cmd *cmd)
{
	...
	while (sectors) {
		...
		/* Always in 512 byte units for Fikus/Block */
		  ...
		  enum dma_data_direction data_direction)
{
	 ...
	 * units unconditionally used by the Fikus block layer.
	 */
...
MODULE_AUTHOR("nab@Fikus-iSCSI.org");
MODULE_LICENSE("GPL");
