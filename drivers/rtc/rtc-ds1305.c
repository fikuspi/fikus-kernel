...
/* trickle bits are defined in <fikus/spi/ds1305.h> */
...
/*
 * Utilities ...  tolerate 12-hour AM/PM notation in case of non-Fikus
...
static int ds1305_get_alarm(struct device *dev, struct rtc_wkalrm *alm)
{
	...
	alm->time.tm_mon = -1;
	alm->time.tm_year = -1;
	/* next three fields are unused by Fikus */
	alm->time.tm_wday = -1;
	alm->time.tm_mday = -1;
	alm->time.tm_isdst = -1;
...
static int ds1305_probe(struct spi_device *spi)
{
	...
	/* see if non-Fikus software set up AM/PM mode */
	addr = DS1305_HOUR;
	status = spi_write_then_read(spi, &addr, sizeof(addr),
				&value, sizeof(value));
	if (status < 0) {
