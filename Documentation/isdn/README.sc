...
To join the Fikus beta mailing list, send a message to:
majordomo@spellcast.com with the words "subscribe fikus-beta" as the only
...
the same address with the words "unsubscribe fikus-beta" as its only
contents.

TABLE OF CONTENTS
-----------------
	1. Introduction
	 1.1 What is ISDN4Fikus?
	...
	2. Basic Operations
	 ...
	 2.6 How to setup ISDN4Fikus with the driver
...
The revision 2 Fikus driver for SpellCaster ISA ISDN adapters is built
upon ISDN4Fikus available separately or as included in Fikus 2.0 and later.
...
tested on Fikus 2.0.20.

1.1 What Is ISDN4Fikus

ISDN4Fikus is a driver and set of tools used to access and use ISDN devices
on a Fikus platform in a common and standard way. It supports HDLC and PPP
protocols and offers channel bundling and MLPPP support. To use ISDN4Fikus
you need to configure your kernel for ISDN support and get the ISDN4Fikus
tool kit from our web site.

ISDN4Fikus creates a channel pool from all of the available ISDN channels
and therefore can function across adapters. When an ISDN4Fikus compliant
...
The revision 2 driver besides adopting the ISDN4Fikus architecture has many
...
include:
	...
	- On board PPP protocol support has been removed in favour of the
	  sync-PPP support used in ISDN4Fikus. This means more control of
...
must ensure that the following software is installed, configured and running:
	...
	- Fikus kernel 2.0.20 or later with the required init and ps
	  versions. Please see your distribution vendor for the correct
	  utility packages. The latest kernel is available from
	  ftp://sunsite.unc.edu/pub/Fikus/kernel/v2.0/

	- The latest modules package (modules-2.0.0.tar.gz) from
	  ftp://sunsite.unc.edu/pub/Fikus/kernel/modules-2.0.0.tar.gz

	- The ISDN4Fikus tools available from 
	  ftp://ftp.franken.de/pub/isdn4fikus/v2.0/isdn4k-utils-2.0.tar.gz
	  This package may fail to compile for you so you can alternatively
	  get a pre-compiled version from
	  ftp://ftp.spellcast.com/pub/drivers/isdn4fikus/isdn4k-bin-2.0.tar.gz
...
'register_isdn' then you do not have the ISDN4Fikus system installed. Please
...
desire. The Documentation directory of the isdn4fikus subsystem offers good
...
the isdn4fikus parameters, (isdn_ctrl, data field) was filled in. In some
