...
# 		}
		...
		if ($line =~ /\bFIKUS_VERSION_CODE\b/) {
			WARN("FIKUS_VERSION_CODE",
			     "FIKUS_VERSION_CODE should be avoided, code should be for the version to which it is merged\n" . $herecurr);
		}
...
#warn if <asm/foo.h> is #included and <fikus/foo.h> is available (uses RAW line)
		if ($tree && $rawline =~ m{^.\s*\#\s*include\s*\<asm\/(.*)\.h\>}) {
			my $file = "$1.h";
			my $checkfile = "include/fikus/$file";
			if (-f "$root/$checkfile" &&
			    $realfile ne $checkfile &&
			    $1 !~ /$allowed_asm_includes/)
			{
				if ($realfile =~ m{^arch/}) {
					CHK("ARCH_INCLUDE_FIKUS",
					    "Consider using #include <fikus/$file> instead of <asm/$file>\n" . $herecurr);
				} else {
					WARN("INCLUDE_FIKUS",
					     "Use #include <fikus/$file> instead of <asm/$file>\n" . $herecurr);
				}
			}
		}
...
# make sure symbols are always wrapped with VMFIKUS_SYMBOL() ...
# all assignments may have only one of the following with an assignment:
#	.
#	ALIGN(...)
#	VMFIKUS_SYMBOL(...)
		if ($realfile eq 'vmfikus.lds.h' && $line =~ /(?:(?:^|\s)$Ident\s*=|=\s*$Ident(?:\s|$))/) {
			WARN("MISSING_VMFIKUS_SYMBOL",
			     "vmfikus.lds.h needs VMFIKUS_SYMBOL() around C-visible symbols\n" . $herecurr);
		}
...
# check for lockdep_set_novalidate_class
		if ($line =~ /^.\s*lockdep_set_novalidate_class\s*\(/ ||
			...
			if ($realfile !~ m@^kernel/lockdep@ &&
			    $realfile !~ m@^include/fikus/lockdep@ &&
