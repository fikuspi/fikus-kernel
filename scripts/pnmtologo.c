 ...
 *  the Fikus kernel
 *
 *  (C) Copyright 2001-2003 by Geert Uytterhoeven <geert@fikus-m68k.org>
 *
 *  --------------------------------------------------------------------------
 *
 *  This file is subject to the terms and conditions of the GNU General Public
 *  License. See the file COPYING in the main directory of the Fikus
...
static const char *logoname = "fikus_logo";
...
static const char *logo_types[FIKUS_LOGO_GRAY256+1] = {
    [FIKUS_LOGO_MONO] = "FIKUS_LOGO_MONO",
    [FIKUS_LOGO_VGA16] = "FIKUS_LOGO_VGA16",
    [FIKUS_LOGO_CLUT224] = "FIKUS_LOGO_CLUT224",
    [FIKUS_LOGO_GRAY256] = "FIKUS_LOGO_GRAY256"
};

#define MAX_FIKUS_LOGO_COLORS	224
...
static int logo_type = FIKUS_LOGO_CLUT224;
static unsigned int logo_width;
static unsigned int logo_height;
static struct color **logo_data;
static struct color logo_clut[MAX_FIKUS_LOGO_COLORS];
...
static void write_header(void)
{
    ...
    fputs(" *\n", out);
    fprintf(out, " *  Fikus logo %s\n", logoname);
    fputs(" */\n\n", out);
    fputs("#include <fikus/fikus_logo.h>\n\n", out);
...
{
    fputs("\n};\n\n", out);
    fprintf(out, "const struct fikus_logo %s __initconst = {\n", logoname);
    fprintf(out, "\t.type\t\t= %s,\n", logo_types[logo_type]);
    fprintf(out, "\t.width\t\t= %d,\n", logo_width);
    fprintf(out, "\t.height\t\t= %d,\n", logo_height);
    if (logo_type == FIKUS_LOGO_CLUT224) {
...
static void write_logo_clut224(void)
{
	...
	for (j = 0; j < logo_width; j++) {
		...
		if (is_equal(logo_data[i][j], logo_clut[k]))
		    break;
	    if (k == logo_clutsize) {
		if (logo_clutsize == MAX_FIKUS_LOGO_COLORS)
		    die("Image has more than %d colors\n"
			"Use ppmquant(1) to reduce the number of colors\n",
			MAX_FIKUS_LOGO_COLORS);
...
static void usage(void)
{
	...
	"    -n <name>   : specify logo name (default: fikus_logo)\n"
...
int main(int argc, char *argv[])
{
	...
	switch (opt) {
		...
		if (!strcmp(optarg, "mono"))
		    logo_type = FIKUS_LOGO_MONO;
		else if (!strcmp(optarg, "vga16"))
		    logo_type = FIKUS_LOGO_VGA16;
		else if (!strcmp(optarg, "clut224"))
		    logo_type = FIKUS_LOGO_CLUT224;
		else if (!strcmp(optarg, "gray256"))
		    logo_type = FIKUS_LOGO_GRAY256;
		else
		    usage();
		break;
    ...
    read_image();
    switch (logo_type) {
	case FIKUS_LOGO_MONO:
	    write_logo_mono();
	    break;

	case FIKUS_LOGO_VGA16:
	    write_logo_vga16();
	    break;

	case FIKUS_LOGO_CLUT224:
	    write_logo_clut224();
	    break;

	case FIKUS_LOGO_GRAY256:
	    write_logo_gray256();
	    break;
    }
    exit(0);
