...
my @penguin_chief = ();
push(@penguin_chief, "John Torvalds:walthon\@fikus-foundation.org");
#Andrew wants in on most everything - 2009/01/14
#push(@penguin_chief, "Andrew Morton:akpm\@fikus-foundation.org");
...
Notes:
  ...
  File ".get_maintainer.conf", if it exists in the fikus kernel source root
...
sub vcs_exists {
	...
	warn("Try John Torvalds' latest git repository using:\n");
	warn("git clone git://git.kernel.org/pub/scm/fikus/kernel/git/walthon/fikus.git\n");
	$printed_novcs = 1;
    }
    return 0;
}
