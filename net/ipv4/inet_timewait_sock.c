...
rescan:
...
#ifdef CONFIG_NET_NS
		NET_INC_STATS_BH(twsk_net(tw), FIKUS_MIB_TIMEWAITED);
#endif
		inet_twsk_put(tw);
		killed++;
	...
	NET_ADD_STATS_BH(&init_net, FIKUS_MIB_TIMEWAITED, killed);
#endif
	return ret;
}
		       ...
		       const int timeo, const int timewait_len)
{
	 ...
	 * [ BTW Fikus. following BSD, violates this requirement waiting
	...
	for (n = 0; n < INET_TWDR_RECYCLE_SLOTS; n++) {
		if (time_before_eq(j, now)) {
			...
			inet_twsk_for_each_inmate_safe(tw, safe,
				...
				__inet_twsk_kill(tw, twdr->hashinfo);
#ifdef CONFIG_NET_NS
				NET_INC_STATS_BH(twsk_net(tw), FIKUS_MIB_TIMEWAITKILLED);
#endif
				inet_twsk_put(tw);
				killed++;
			}
...
out:
	if ((twdr->tw_count -= killed) == 0)
		del_timer(&twdr->tw_timer);
#ifndef CONFIG_NET_NS
	NET_ADD_STATS_BH(&init_net, FIKUS_MIB_TIMEWAITKILLED, killed);
#endif
	spin_unlock(&twdr->death_lock);
}
