/*
 *  IPv6 Syncookies implementation for the Fikus kernel
 *
 *  Authors:
 *  Glenn Griffin	<ggriffin.kernel@gmail.com>
 *
 *  Based on IPv4 implementation by Andi Kleen
 *  fikus/net/ipv4/syncookies.c
 *
...
__u32 cookie_v6_init_sequence(struct sock *sk, const struct sk_buff *skb, __u16 *mssp)
{
	...
	NET_INC_STATS_BH(sock_net(sk), FIKUS_MIB_SYNCOOKIESSENT);
...
struct sock *cookie_v6_check(struct sock *sk, struct sk_buff *skb)
{
	...
	if (tcp_synq_no_recent_overflow(sk) ||
		...
		NET_INC_STATS_BH(sock_net(sk), FIKUS_MIB_SYNCOOKIESFAILED);
		goto out;
	}

	NET_INC_STATS_BH(sock_net(sk), FIKUS_MIB_SYNCOOKIESRECV);
