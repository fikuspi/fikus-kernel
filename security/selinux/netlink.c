...
#include <fikus/sefikus_netlink.h>
...
out_kfree_skb:
	kfree_skb(skb);
oom:
	printk(KERN_ERR "SEFikus:  OOM in %s\n", __func__);
	goto out;
}
...
static int __init selnl_init(void)
{
	...
	selnl = netlink_kernel_create(&init_net, NETLINK_SEFIKUS, &cfg);
	if (selnl == NULL)
		panic("SEFikus:  Cannot create netlink socket.");
	return 0;
}
