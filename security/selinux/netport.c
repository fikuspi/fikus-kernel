 ...
 * Network port table
 *
 * SEFikus must keep a mapping of network ports to labels/SIDs.  This
 ...
 *   (see security/sefikus/netif.c for more information)
 *
 */
...
out:
	...
	if (unlikely(ret)) {
		printk(KERN_WARNING
		       "SEFikus: failure in sel_netport_sid_slow(),"
